require([
    ...
    "esri/config", "esri/Map", "esri/views/MapView", "esri/widgets/BasemapToggle", "esri/layers/FeatureLayer", "esri/PopupTemplate", "esri/widgets/Search", "esri/widgets/Legend", "esri/widgets/Expand", "esri/layers/GraphicsLayer","esri/widgets/Sketch", "esri/widgets/Sketch/SketchViewModel", "esri/widgets/LayerList","esri/widgets/Editor"

], function (

    esriConfig, Map, MapView, BasemapToggle, FeatureLayer, PopupTemplate, Search, Legend, Expand, GraphicsLayer, Sketch, SketchViewModel,LayerList,Editor
    ...
    var popupReclamation = new PopupTemplate({
        ...
        content: "Objet : {Objet} <br> Message : {Message} <br> Email : {Mail}"
    });
...
view.ui.add(basemapExpand, "top-left");
    ...
    document.getElementById("addReclamation").addEventListener("click", function(event) {
...
reclamation.on("apply-edits", function() {
    loadReclamations();
});
*/


    // 3️⃣ Configuration de l'outil d'édition Editor
    const editor = new Editor({
        view: view,
        layerInfos: [{
            layer: reclamation,
    ...
    // Ajout de l'outil d'édition à l'interface
    ...
    // 4️⃣ Fonction pour récupérer et afficher la liste des réclamations
    function loadReclamations() {
        reclamation.queryFeatures({
            where: "1=1",
            outFields: ["Objet", "Message", "Mail"],
